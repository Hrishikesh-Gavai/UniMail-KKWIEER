  ...
  const downloadExcel = async () => {
    try {
      ...
      const excelData = recordsWithUrls.map(record => ({
        ...
        'Download PDF': record.pdf_filename ? `➕ ${record.pdf_filename}` : 'No PDF',
      ...
      recordsWithUrls.forEach((record, index) => {
        if (record.pdfUrl && record.pdf_filename) {
          const cellAddress = XLSX.utils.encode_cell({ r: index + 1, c: 10 });
          
          worksheet[cellAddress] = {
            t: 's',
            v: `➕ ${record.pdf_filename}`,
            l: { 
              Target: record.pdfUrl, 
              Tooltip: `Click to download ${record.pdf_filename}` 
            }
          };
        }
      });
